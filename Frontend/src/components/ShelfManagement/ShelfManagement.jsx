        ...
        {shelves.map((shelf) => (
          <div key={shelf.id}>
            {editingShelfId === shelf.id ? (
              ...
              <div>
                <p>
                  Delete "{shelf.name}" and move its books to:
                </p>

                <select
                  value={destinationShelfId}
                  onChange={(e) =>
                    setDestinationShelfId(e.target.value)
                  }
                >
              ...
              <div>
                ...
                <button onClick={() => startRename(shelf)}>
                  Rename
                </button>

                {/* Cannot delete the only shelf */}
                {shelves.length > 1 && (
                  <button onClick={() => startDelete(shelf)}>
                    Delete
                  </button>
                )}
              </div>
